...
各パラメータの意味・役割・調整指針をまとめた資料。先方への仕様説明用。

> 対応実装：`emg_sim/dsp/`（信号処理・正規化）、`emg_sim/control/`（マッピング・IK）、
> `emg_sim/config.py`（全パラメータの既定値の SSoT）。設定はアプリの設定ウィンドウ（**S** キー）で
> スライダ調整＋JSON 保存できる。**具体的な数値は config／設定ウィンドウが常に最新**なので、
> 本資料は各値の「意味と調整の向き」を示すに留める（数値の二重管理・ドリフトを避けるため）。
...
- `sample_rate`（標準 1000 Hz）は、以降の「ms 指定」を内部でサンプル数に換算する基準。
...
- `ema_alpha` が大きいほど即応（平滑弱）、小さいほど滑らか（平滑強）。
...
- `scale` の初期値・下限は `fallback_scale`。誰も較正しなくても最低限動く保険で、
  **弱い信号の人が端まで届かない場合は下げる**（＝除数を下げて高感度に）。設定ウィンドウで調整可。
...
- `sat_gain` は tanh の**急峻さ**。大＝軽い力で最大近くまで／小＝じっくり微調整。
...
### 4.1 ソフト飽和カーブ（例：`sat_gain = 1.6`）

`a_pre = x/scale`（力み比。1.0 ≒ 満力）に対する活性度 `a = tanh(1.6·a_pre)`（`sat_gain=1.6` の例）：
...
実際の `sat_gain` は設定ウィンドウで調整でき、スライダ下にこのカーブが即時プレビューされる。
...
- **跳ね上がりは速く、減衰は遅い**（半減期 `peak_halflife_sec`、回復は概ねその2倍）。
...
- **到達判定は目標を中心とした小さな (r,θ) の楔**（半径方向 `±reach_r`、角度方向 `±reach_theta_deg`）に
  入って `hold_sec` 保持で成立。r と θ の許容を**独立に**指定できるのがポイントで、θ は指先の移動量が
  弧長 `r·Δθ`（θ範囲180°では半周 `πr`）と大きく **r より高ゲイン**なため、角度許容だけを別に緩めて
  左右の難易度を揃えられる（円判定だと外側の目標ほど θ がシビアになる問題を解消）。
...
```
   ...
   ┌──────┼──────┐   外弧 = r_max
   │ ░░░░░░░░░░░ │
   │ ░ 目標生成 ░ │   ← target_margin だけ内側／目標は (r,θ) の楔
   │ ░░░░░░░░░░░ │
   └──────┼──────┘   内弧 = r_min
  θ=180°  ●  θ=0°     ● = アーム基部（原点）
        （台座）
...
> **具体的な数値は載せない**（`config.py` と設定ウィンドウ **S** が最新の SSoT）。ここでは各値の
> **意味と調整の向き**だけを示す。値の二重管理でドリフトするのを避けるため。

### 信号処理（`SignalConfig`）

| 名前 | 変数 | 効果／調整の向き |
|---|---|---|
| サンプリング | `sample_rate` | ms→サンプル換算の基準。実機のレートに合わせる |
| RMS 窓 | `rms_window_ms` | 小=即応/ビクビク、大=滑らか/遅い（「即応↔安定」の主ツマミ） |
| EMA α | `ema_alpha` | 大=即応、小=滑らか（1.0=EMA無効） |
| 帯域通過 | `bp_low`/`bp_high` | 低域ドリフト・帯域外ノイズ除去（実EMG用に 20–450 Hz） |
| ノッチ | `notch_freq` | 電源ハム除去（地域で 50 or 60 Hz） |

### 正規化（`NormalizeConfig`）

| 名前 | 変数 | 効果／調整の向き |
|---|---|---|
| 較正時間 | `baseline_sec` | 脱力ベースライン取得の秒数 |
| ソフト飽和 gain | `sat_gain` | 大=軽い力で上限へ、小=微調整しやすい |
| 適応率 | `adapt_rate` | scale が peak を追う速度（0=固定） |
| peak 半減期 | `peak_halflife_sec` | 大=基準が長く残る／小=速く戻る（回復 ~2倍） |
| フォールバック | `fallback_scale` | scale の初期値・下限。弱い信号の人は下げる＝高感度 |

### マッピング・幾何（`ControlConfig`）

| 名前 | 変数 | 効果／調整の向き |
|---|---|---|
| リーチ範囲 | `r_min` / `r_max` | 到達域の内外半径（m） |
| 旋回範囲 | `θ_min` / `θ_max` | 扇の角度範囲 |
| 操作平面高さ | `z_plane` | 肩の高さ＝最大到達 |
| 満力基準 | `reach_full_activation` | この活性度で r_max に届く再スケール |
| 肘（近/遠） | `elbow_target_near`/`far` | 近=畳む、遠=伸ばす（r で補間） |
| 手先下向き | `tool_down` / `_gain` | マニピュレータを目標に下向きに |

### ゲーム（`GameConfig`）

| 名前 | 変数 | 効果／調整の向き |
|---|---|---|
| 到達許容 r | `reach_r` | 目標中心からの半径方向の許容（m） |
| 到達許容 θ | `reach_theta_deg` | 同・角度方向の許容（度）。θ は高ゲインなので r と独立に緩められる |
| 滞在時間 | `hold_sec` | 連続滞在でクリア（狙って止める要求） |
| 端マージン | `target_margin` | 扇の端を除外して目標生成 |
| 1ラウンド | `targets_per_round` | 規定個数の到達でタイム→リセット |
...
1. **RMS 窓**で「ガタつき ↔ 遅れ」を合わせる（ビクビクにするなら短め 10–30ms）。
2. **ソフト飽和 gain**で「軽さ ↔ 精密さ」を合わせる。
3. **適応率**は基本そのまま（振り切れる人が多ければ少し上げる、0 で固定）。
4. **peak 半減期**で「基準がどれだけ長く残るか」を調整（回復を速めたいなら下げる）。
5. **r_min/r_max・θ 範囲**を実機の可動域に合わせる。
6. **到達許容（r/θ）・滞在時間**で難易度を合わせる（θ を広めにすると左右の難しさが揃う。展示は甘めが無難）。
7. **フォールバック（`fallback_scale`）**は、弱くて端まで届かない人が出たら下げる。
