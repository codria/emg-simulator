...
`scale` は固定ではなく、**セッション中の観測ピークに向けて上方向・低速に自動追従**する
（＝EMA と同じ指数平滑）。較正が甘い人（人前で5割）でも、本気を出した瞬間に追従して振り切れを防ぐ。

```
peak  = max(peak, x)                        # 観測ピーク（x の走行最大, 単調増加）
scale += adapt_rate · max(0, peak − scale)  # scale を peak へ EMA で上方向追従
a_pre = x / scale                           # 満力で ≈1.0 になる正規化比
```

- `scale` の初期値は `fallback_scale`（既定 0.5）。誰も較正しなくても最低限動く保険。
- 弱い人（peak < scale）は fallback のまま、強い人は scale が追従。**下方向には動かさない**（安定のため）。
...
② 正規化
   ...
   peak  = max(peak, x)
   scale += adapt_rate · max(0, peak − scale)
...
### 4.2 オンライン適応（peak は段差／scale は平滑）

力み `x` が急に上がったとき（例：`peak` が 0.5→1.0）、`scale` は **一気に飛ばず EMA でじわっと**追従：

<!--FIG-REPLACE adapt 13-->
```
x     ┈┈╱▔▔▔▔▔▔▔  (急に力む)
peak  ┈┈┈▔▔▔▔▔▔▔  (最大保持＝段差で上がる)
scale ┄┄┄╱‾‾‾‾‾‾  (peak を追う EMA＝なめらかに上がる, adapt_rate)
```

| frame | peak | scale（adapt_rate=0.05） |
|---|---|---|
| 0 | 0.50 | 0.500 |
| 1 | 1.00 | 0.525 |
| 2 | 1.00 | 0.549 |
| … | 1.00 | じわじわ 1.0 へ（時定数 ~0.3 s） |

**なぜ平滑にするか**：scale を即座に peak に一致させると感度が急変して操作感が破綻するため、
なめらかに上げて感度変化を緩やかにする。
...
| 適応率 | `adapt_rate` | 0.05 | 0–0.2 | scale の追従速度（0=固定） |
| フォールバック | `fallback_scale` | 0.5 | — | 未較正時の初期 scale（保険） |

### マッピング・幾何（`ControlConfig`）

| 名前 | 変数 | 既定 | 効果 |
|---|---|---|---|
| リーチ範囲 | `r_min` / `r_max` | 0.15 / 0.70 | 到達域の内外半径（m） |
...
| `peak` | `x` の走行最大値（観測ピーク、単調増加、段差状に上がる） |
| `scale` | 満力基準の除数。`peak` を EMA で上方向追従（なめらか） |
...
4. **r_min/r_max・θ 範囲**を実機の可動域に合わせる。
5. **到達距離・滞在時間**で難易度を合わせる（展示は甘めが無難）。
