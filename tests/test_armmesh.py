...
def _unit_normals(n, v):
    return n.shape == v.shape and np.allclose(np.linalg.norm(n, axis=1), 1.0, atol=1e-6)


def test_cylinder_within_radius():
    v, n = armmesh.cylinder_tris(0.05, 0.2, 24)
    assert _valid_soup(v)
    assert _unit_normals(n, v)               # one unit normal per vertex
...
def test_curved_bone_endpoints():
    ...
    v, n = armmesh.curved_bone_tris(p0, (0, 0, 0.12), (0, -0.05, 0.18), p3, 0.03)
    assert _valid_soup(v)
    assert _unit_normals(n, v)
...
def test_build_parts_shape():
    ...
    for p in parts:
        ...
        assert _unit_normals(p["normals"], p["verts"])
